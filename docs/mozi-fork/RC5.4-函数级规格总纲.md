...
>
> 上位文档：`自我进化机制-RC5.4-方案.md`；本总纲 + 五份附件把方案落到**类名 / 函数名 / 签名 / 调用关系 / 验收标准**级，按 TDD 实施
>
> 附件索引：`RC5.4-附件P0-evidence-lock.md`、`RC5.4-附件P1-memory.md`、`RC5.4-附件P2-skill-managed.md`、`RC5.4-附件P3-session-review.md`、`RC5.4-附件P4-curator.md`（P5 rollout 无独立代码面，见本文件 §8）
>
> 第六轮评审处置：`RC5.4-第六轮评审核验与处置.md`（12 S1 全部证实，逐条落点见其 §4）
>
> 日期：2026-08-29
...
- **身份规则**：`ProjectKey = hash(ctx.fs.resolve(root).targetKey)`（键 branded 禁解析，`fs/fs/src/types.ts:8-15`）；`SkillId = hash(ProjectKey, normalizedName)`；`ReviewRangeId`/`ReviewAttemptId = hash(rangeId, attemptNo)`——全部纯函数可推导，不解析不拼路径。
...
| `packages/memory/memory` | **单一** `MemoryService extends Service`（`getState(scope)`/`applyOps(scope)`/`acknowledgeTerminalOps(opIds)`）；composite `MemoryPublisher`；`foldMemoryOps`/`enforceBudget`/`sanitizeForPublication`/`buildSnapshotSections`/`computeSnapshotDigest` | 双逻辑 scope 权威状态 + receipt 二分保留 + sanitize→render→digest 发布（fail-open） | P1 |
| `packages/skill/skill-managed` | **`ManagedSkillService extends Service`**（唯一 domain owner：ManagedSkillStore/NameIndex/ManagedSkillProvider/AuthoringCore + `MANAGED_SKILL_PROVIDER_NAME` 常量）；named export `skill_manage` 工具插件 | agent 自治技能：storage-only catalog、locator 钉 revision/digest、pendingRevision 治理、rejected/reopen、配额 | P2 |
...
memory.MemoryService（唯一 memory 域 opener）
  ...
  ├─ acknowledgeTerminalOps(opIds) ◄── session-review（attempt terminal 时）
...
skill-managed.ManagedSkillService（唯一 dsh.skill-managed 域 opener；provider 注册 global 层）
  ...
  │     ├─ list：sidecar catalogSummary（storage only；损坏 → complete:false）
...
session-review.ReviewRuntime
  ...
  ├─ ctx.memory.applyOps / acknowledgeTerminalOps
...
session-review.governance（宿主命令：list/show/approve/reject/reopen）
  └─ approve 双语义：draft→active；active pending→切 pointer（全重验）
...
| 阶段 | 附件 | 规模 | 核心交付 |
|---|---|---|---|
| P0 | 附件P0 | 53 活跃 + 2 历史 + Hermes 锚点 | 行为事实钉死 + E0 结案 |
| P1 | 附件P1 | 1 Service + 1 Publisher + 12 函数 + content-scan | 双 scope 幂等 + receipt 二分/ack + composite 发布 |
| P2 | 附件P2 | 1 Service（含 Provider/Store/AuthoringCore）+ 12 函数 + 1 工具 | skill-managed 一步到位：storage-only catalog + pendingRevision + rejected/reopen + 配额 |
| P3 | 附件P3 | 3 类 + 15 函数 | ReviewRuntime 全链（attempt 简化/consolidation 新 attempt/scope backstop）+ 治理双语义 + session-query 默认过滤 |
...
| `invalid_structure` | authoring validateStructure / reconcile | frontmatter/字节上限/路径越界/binary/symlink/pointer 指缺失 revision |
