...
>
> 方案状态：RC5.5.5 Architecture Frozen / Implementation Approved。这只表示函数级规格、调用图与验收门已闭合，不表示 P1–P5 已实现。P1-R3必须替换批C的 append-only Publisher；P2–P5必须按owner/promotion、named reviewer、repair corroboration、bounded consolidation和eval v2实施。
...
2. `自我进化机制-RC5.5-方案.md`：RC5.5.5 上位原则、数据模型、运行协议和 Phase。
3. `RC5.5-函数级规格总纲.md`：包 DAG、跨包调用图、错误码与 T69–T94。
4. 当前 Phase 附件；现在是 `RC5.5-附件P1-memory.md`。
5. 第十一轮四项裁定读 `RC5.5.5-第十一轮评审核验与处置.md`；第十轮 current-surface/attestation/outcome fault/orphan 处置继续有效，第九轮文件是历史输入。
...
- unresolved、assistant-only 和 Host 不能验证的结论不得产生可见 memory/skill；tool-success 仅是 execution 成功，不是任务语义成功。同invocation重试成功是`retry-recovered`；changed invocation只形成非因果`RepairEpisode`，单episode只能支持隐藏agent draft。User confirmation必须是human-only command产生、绑定exact lesson/ref/revision/digest的durable operation，planner不能从普通文本自行断言。
...
- conservative review先选load-time named `ReviewExecutionProfile`，再在 claim前重算并验证 `ReviewAuthorizationScopeDigest`；historical source route只作provenance，不决定profile/scope/lane，cold carrier不运行source turn。未授权execution只进 shadow；actual child request attestation在 plan/mutation前验证。planner只见 scoped `structured_output`且恰好成功一次，普通工具与 inherited runtime context为零。
...
- Agent-owned skill只有在Host opt-in、未pinned、强evidence、exact base与`conservative-auto` capability均通过时才能自动active；user-owned永不auto。Background activation OpId由Host从actor/attempt/exact candidate稳定派生且不含artifact digest，lineage与current pointer同笔CAS，恢复先做exact duplicate查找；governance lineage没有permit字段。P4b只处理bounded cluster，destination的强evidence必须由Host从stored attempt + successful exact preflight派生并先持久化；P4在promotion前重读attempt核对plan/source/preflight，P2只拥有candidate/current/permit检查，不能反向导入P4。同scope还必须同时授权`skill-auto-promotion`与`skill-consolidation`。先使destination active，再按exact source base记录absorption/archive，source bundle保留可恢复；Host evidence不声称语义保真。
- P5 hard gate不得被人工豁免；每个 named execution scope独立评测并签署，pass也不拷贝 shadow plan，而是新 draft/auto lane重审。Eval protocol v2九分层每scope至少333个held-out case，其中changed-method/consolidation各至少35，v1报告不兼容；隔离eval permit + 分离的process-local promotion/consolidation authorities不能进production。重新签署同一 scope/level不制造新 lane。

## 5. 验收与文档地图

P0继续保持 T01–T68 test-tree事实回归；RC5.5.5生产验收为 T69–T94，T85/T86取代 T67/T68 reference的生产顺序/分类，T87–T90闭合第十轮协议，T91–T94闭合promotion、named reviewer、consolidation与repair learning。验收名是待实现的行为清单，不是测试数量上限；每个 parser/config/durable/filesystem/model/tool/wire边界都要有 invalid case。

| 文档 | 角色 |
|---|---|
| `RC5.5-执行进度报告.md` | 唯一已提交进度台账与下一步 |
| `自我进化机制-RC5.5-方案.md` | RC5.5.5 上位方案 |
| `RC5.5-函数级规格总纲.md` | 全局 DAG、调用图、错误码、T69–T94、Phase 门 |
| `RC5.5-附件P0-evidence-lock.md` | 既有 API 事实与 68 项 P0 矩阵 |
| `RC5.5-附件P1-memory.md` | 当前 Phase 的 D01–D19 函数级规格 |
| `RC5.5-附件P2-skill-managed.md` | identity/lineage/preflight/owner/promotion/absorption/provider/tool |
| `RC5.5-附件P3-session-review.md` | named-profile Host review、repair、history、governance、provenance |
| `RC5.5-附件P4-curator.md` | P4a signal/coverage/lifecycle + P4b bounded consolidation |
| `RC5.5-附件P5-rollout.md` | eval v2 corpus/runner/scorer/draft-auto gate/report |
| `RC5.5.5-第十一轮评审核验与处置.md` | 四项裁定、修复取舍与新版自审 |
| `RC5.5.4-第十轮评审核验与处置.md` | 继续有效的current-surface/attestation/outcome fault/orphan处置 |
